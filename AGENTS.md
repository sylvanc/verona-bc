...
- **Identified TypeVars + constraint solver**: every `TypeVar` AST node carries a unique source `Location` (its identity). A `TypeName` resolving to a `TypeParam` is also treated as a TypeVar identity by the subtype machinery, using the `TypeParam`'s `Ident` Location — no AST mutation is needed. `vc::TypeVarStore::global()` is a singleton store with union-find equivalence, lower/upper bound vectors, occurs check, and solve (LUB/GLB). `SequentCtx` carries `void* constraint_store` + `Mode {Query, Emit}`. In `Mode::Emit`, atom rules wrapped via `with_typevar(...)` (vc/subtype.h) emit `add_lower` / `add_upper` / `unify` and return true (delayed proof). Read-only subtype callers stay in `Mode::Query`; emission paths (call-arg checks in infer's Call/CallDyn handlers; `emit_source_call_constraints` pre-pass in reify) flip to `Emit`. Reify seeds `r.subst[TypeParam] = Type<TypeVar>` at the TypeParam's Ident Location for unbound formals; `apply_subst` no longer skips TypeVar-valued entries, so the seed flows through. Solver consumption fills `r.subst` from `solve(α_k)`. Bottom (`solve` returns empty `Union`) is sound for zero-evidence formals (e.g. `f[T](): T | none = { none }`). After all reifications, an IR-boundary scan walks the emitted IR for free TypeVar leaves and emits a hard error for any survivors.
