...
### Known issue: finalization ordering
The current implementation doesn't find the transitive closure of objects being collected before finalizing. The correct approach would be: find all objects to collect, finalize them all, THEN deallocate them all. This prevents use-after-free when finalizers reference objects that are also being collected.
