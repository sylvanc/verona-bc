  ...
  PassDef statements()
  {
    return {
      ...
      dir::bottomup,
      {
        ...
        // Primitive class.
        (T(Primitive) << End) * PrimitiveType[Type] * T(GlobalId)++[GlobalId] >>
          [](Match& _) {
            return Primitive << (Type << _(Type)) << (Funcs << _[GlobalId]);
          },

        // User-defined class.
        (T(Class) << End) * T(GlobalId)[GlobalId] >>
          [](Match& _) { return Class << _(GlobalId) << Fields << Funcs; },
        ...
        (T(Class) << T(GlobalId))[Class] * T(GlobalId)[GlobalId] >>
          [](Match& _) {
            (_(Class) / Funcs) << _(GlobalId);
            return _(Class);
          },
