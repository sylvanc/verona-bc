  ...
  PassDef infer()
  {
    ...
    p.post([](auto top) {
      top->traverse([&](auto node) {
        ...
        for (auto& lbl : *labels)
        {
          for (auto& stmt : *(lbl / Body))
          {
            ...
            else if (stmt->in({NewArray, NewArrayConst}))
            {
              ...
              // NewArrayConst is exclusively generated by ANF tuple
              // lowering. Track it for TupleType construction.
              if (stmt == NewArrayConst)
              {
                auto sz_node = stmt / Rhs;
                auto sz = from_chars_sep_v<size_t>(sz_node);
                tuple_locals[dst->location()] = {stmt, sz, {}};
        ...
        // Finalize tuple types: for each tracked NewArrayConst where
        // all element types were resolved, replace the Type child with
        // a TupleType node so reify can preserve per-element types.
