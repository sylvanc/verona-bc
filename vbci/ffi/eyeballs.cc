...
namespace vbci
{
  struct Eyeballs
  {
    ...
    void resolved(int status, struct addrinfo* res)
    {
      if (status < 0)
      {
        Thread::run_sync(cb, ValueWithoutRC(arg), ValueWithRC(Value::null()), ValueWithRC(status));
        return;
      }
    ...
    void connect(uv_connect_t* req, int status)
    {
      ...
      if ((status == 0) && !winner)
      {
        ...
        close_all();
        Thread::run_sync(cb, ValueWithoutRC(arg), ValueWithRC(winner), ValueWithRC(status));
      }
