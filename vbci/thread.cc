...
namespace vbci
{
  ...
  struct Operands
  {
    ...
    register_ref(Thread& self, F fun, Args... args)
    {
      ...
      self.trace_instruction(" Local=", reg_index, " (", reg.borrow(), ")");
      process<F, Args..., Register&>(
        self, fun, std::forward<Args>(args)..., reg);
      LOG(Trace) << "Updated " << reg_index << " to " << reg.borrow();
    }
  ...
  Register Thread::thread_run(Function* func)
  {
    ...
    LOG(Trace) << "thread_run completed! Result: " << locals.at(0).borrow();
  ...
  void Thread::print_stack(logging::Log& log, bool top_frame_only)
  {
    ...
    for (ssize_t i = frames.size() - 1; i >= 0; i--)
    {
      ...
      for (size_t j = 0; j < frame.func->registers; j++)
      {
        if (frame.local(j)->is_invalid())
          continue;
        log << std::endl << "    local[" << j << "] = " << frame.local(j).borrow();
      }
      if (top_frame_only)
        break;
      if (i != 0)
    ...
    {
      log << std::endl << "    arg[" << i << "] = " << frame->arg(i).borrow();
    }
  }
      ...
      switch (op)
      {
        ...
        case Op::StoreMove:
        {
          process([](Register& dst, const Register& ref, Register src)
                    INLINE { dst.from_store<true>(ref, std::move(src)); });
          break;
        }
        ...
        case Op::StoreCopy:
        {
          process([](Register& dst, const Register& ref, const Register& src)
                    INLINE { dst.from_store<false>(ref, src); });
          break;
        }
